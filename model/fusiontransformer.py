...
class TransfuserBackbone(nn.Module):
    ...
    def __init__(self, config, lidar_architecture='resnet18', use_velocity=True):
        super().__init__()
        self.config = config
        ...
        image_architecture = self.config.image_architecture
    ...
    def forward(self, image, lidar, velocity):
        ...
        event_features = self.image_encoder.features.conv1(event_tensor)
        event_features = self.image_encoder.features.bn1(event_features)
        event_features = self.image_encoder.features.act1(event_features)
        event_features = self.image_encoder.features.maxpool(event_features)
        ...
        event_features = self.image_encoder.features.layer1(event_features)

        # Image fusion at (B, 72, 40, 176)
        # Lidar fusion at (B, 72, 64, 64)
        image_embd_layer1 = self.avgpool_img(image_features)
        event_embd_layer1 = self.avgpool_lidar(event_features)


        ##要改transformer1
        image_features_layer1, event_features_layer1 = self.transformer1(image_embd_layer1, event_embd_layer1, velocity)
        image_features_layer1 = F.interpolate(image_features_layer1,
                                              ...
                                              align_corners=False)
        event_features_layer1 = F.interpolate(event_features_layer1,
                                              size=(lidar_features.shape[2], lidar_features.shape[3]), mode='bilinear',
                                              align_corners=False)
        image_features = image_features + image_features_layer1
        event_features = event_features + event_features_layer1

##################2
        image_features = self.image_encoder.features.layer2(image_features)
        event_features = self.image_encoder.features.layer2(event_features)

        # Image fusion at (B, 216, 20, 88)
        # Image fusion at (B, 216, 32, 32)
        image_embd_layer2 = self.avgpool_img(image_features)
        event_embd_layer2 = self.avgpool_lidar(event_features)
        image_features_layer2, event_features_layer2 = self.transformer2(image_embd_layer2, event_embd_layer2, velocity)
        image_features_layer2 = F.interpolate(image_features_layer2,
                                              size=(image_features.shape[2], image_features.shape[3]), mode='bilinear',
                                              align_corners=False)
        event_features_layer2 = F.interpolate(event_features_layer2,
                                              size=(lidar_features.shape[2], lidar_features.shape[3]), mode='bilinear',
                                              align_corners=False)
        image_features = image_features + image_features_layer2
        event_features = event_features + event_features_layer2

        ##################2
        image_features = self.image_encoder.features.layer3(image_features)
        event_features = self.image_encoder.features.layer3(event_features)
        # Image fusion at (B, 576, 10, 44)
        # Image fusion at (B, 576, 16, 16)
        image_embd_layer3 = self.avgpool_img(image_features)
        event_embd_layer3 = self.avgpool_lidar(event_features)
        image_features_layer3, event_features_layer3 = self.transformer3(image_embd_layer3, event_embd_layer3, velocity)
        image_features_layer3 = F.interpolate(image_features_layer3,
                                              size=(image_features.shape[2], image_features.shape[3]), mode='bilinear',
                                              align_corners=False)
        event_features_layer3 = F.interpolate(event_features_layer3,
                                              size=(lidar_features.shape[2], lidar_features.shape[3]), mode='bilinear',
                                              align_corners=False)
        image_features = image_features + image_features_layer3
        event_features = lidar_features + event_features_layer3


#####################4
        image_features = self.image_encoder.features.layer4(image_features)
        event_features = self.image_encoder.features.layer4(event_features)

        # Image fusion at (B, 1512, 5, 22)
        # Image fusion at (B, 1512, 8, 8)
        image_embd_layer4 = self.avgpool_img(image_features)
        event_embd_layer4 = self.avgpool_lidar(event_features)

        image_features_layer4, event_features_layer4 = self.transformer4(image_embd_layer4, event_embd_layer4, velocity)
        image_features_layer4 = F.interpolate(image_features_layer4,
                                              size=(image_features.shape[2], image_features.shape[3]), mode='bilinear',
                                              align_corners=False)
        event_features_layer4 = F.interpolate(event_features_layer4,
                                              size=(lidar_features.shape[2], lidar_features.shape[3]), mode='bilinear',
                                              align_corners=False)
        image_features = image_features + image_features_layer4
        event_features = event_features + event_features_layer4

        # Downsamples channels to 512
        image_features = self.change_channel_conv_image(image_features)
        event_features = self.change_channel_conv_lidar(event_features)
        ...
        event_features = self.lidar_encoder._model.global_pool(event_features)
        event_features = torch.flatten(event_features, 1)

        fused_features = image_features + event_features

        features = self.top_down(x4)
        flow = self.decoder(fused_features)
        return features, image_features_grid, fused_features,flow


class FlowDecoder(nn.Module):
    def __init__(self, in_channels):
        super(FlowDecoder, self).__init__()
        # Expansion pathway: upscale the feature map
        self.upconv1 = nn.ConvTranspose2d(in_channels, 256, kernel_size=3, stride=2, padding=1, output_padding=1)
        ...
        # Final convolution to get the flow vectors
        self.flow_conv = nn.Conv2d(32, 2, kernel_size=3, padding=1)  # Output 2 channels for flow vectors (dx, dy)
    ...
    def forward(self, x):
        ...
        flow = self.flow_conv(x)  # Output shape will be (N, 2, H, W), where 2 represents the flow vector components
        return flow
